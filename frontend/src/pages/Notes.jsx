...
const Notes = () => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-100 to-gray-200 px-6 py-8">
      ...
      >
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <button
            ...
            className="cursor-pointer px-4 py-3 rounded-full text-sm font-medium bg-white/10 border border-white/20 text-white hover:bg-white/20 transition flex items-center gap-2"
          >
            📚Your Notes
          </button>
        </div>
      ...
      {loading && ( <motion.div animate={{opacity:[0.4,1,0.4]}} transition={{repeat:Infinity, duration:1.2}} className="text-center text-black font-medium mb-6" >Generating exam-focused notes...</motion.div> )}
      
      {error && <div className="mb-6 text-center text-red-600 font-medium">{error}</div> }
      ...
      {result && <motion.div initial={{opacity:0, Y:30}} animate={{opacity:1, y:0}} transition={{duration:0.4}} className="flex flex-col lg:grid lg:grid-cols-4 gap-6">
